...
function create (context) {
  ...
  utils.registerTemplateBodyVisitor(context, {
    "VElement" (node) {
      ...
      const attributesCount = node.startTag.attributes.length
      if (attributesCount >= counts) {
        if (node.startTag.loc.end.line - node.startTag.loc.start.line !== attributesCount + 1) {
          // 属性超过 ${counts} 个则需要换行
          context.report({
            node,
            loc: node.loc,
